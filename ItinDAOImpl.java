...
public class ItinDAOImpl implements ItinerariDAO {
	...
	//Diego 10/12 cambio la consulta para que muestre el nombre del itinerario
	
	public Itinerari findById(Integer id) {
		try {
			//String sql = "SELECT * FROM ITINERARI WHERE ID_USER = ?";
			String sql = "select id_attraction, name, cost, duration from itinerari cross join attractions on id_attraction = id where id_user = ?";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
